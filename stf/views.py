...
def register(request):
    ...
    return render(request, 'prod/register.html', {'form': form})
...
def user_login(request):
    ...
    return render(request, 'prod/login.html')

def user_logout(request):
    logout(request)
    messages.info(request, 'You have been logged out.')
    return redirect('register')
from prod.models import Product,Sale
def list_product(request):
    prod = Product.objects.all()
    sale = Sale.objects.all()
    return render(request, 'prod/all.html',{'prod':prod,'sale':sale})

# views.py

from django.utils import timezone
from prod.models import Sale, Product
...
def daily_sales_view(request, date):
    ...
    return render(request, 'prod/daily_sales.html', context)

def list_pro_loan(request):
    prod = Product.objects.filter(is_loan=True)
    return render(request, 'prod/pro_loan.html', {'prod':prod})
   
from prod.models import qrsho_Payment
from prod.forms import qrshoForm
...
def pay_qrsho(request, product_id):
    ...
    return render(request, 'prod/qrsho_pay.html', {'form': form, 'salary': product})
...
def qrsho_payments_view(request, product_id):
    ...
    return render(request, 'prod/qrsho_payments.html', context)
...
def list_qrsho_pay(request, product_id):
    ...
    return render(request, 'prod/list_qrsho_pay.html', context)
...
def sales_dates_view(request):
    ...
    return render(request, 'prod/sales_dates.html', context)
